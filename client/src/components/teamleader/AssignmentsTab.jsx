        ...
        <div style={{ display: 'flex', gap: '0', marginBottom: '18px', background: 'var(--background-secondary)', borderRadius: '10px', padding: '4px', width: 'fit-content' }}>
          ...
          >
            ...
            <span style={{
              ...
              {assignments.filter(a => a.status !== 'completed' && !(a.recent_submissions?.length > 0) && (teamFilter === 'all' || (a.team || 'IT Dept') === teamFilter)).length}
            </span>
          </button>
          <button
            onClick={() => setActiveTaskTab('for_checking')}
            style={{
              padding: '7px 22px', borderRadius: '8px', border: 'none',
              fontWeight: '600', fontSize: '13.5px', cursor: 'pointer',
              transition: 'all 0.18s',
              background: activeTaskTab === 'for_checking' ? 'var(--background-primary)' : 'transparent',
              color: activeTaskTab === 'for_checking' ? 'var(--text-primary)' : 'var(--text-tertiary)',
              boxShadow: activeTaskTab === 'for_checking' ? '0 1px 4px rgba(0,0,0,0.10)' : 'none',
              display: 'flex', alignItems: 'center', gap: '6px',
            }}
          >
            ...
            <span style={{
              marginLeft: '7px', fontSize: '12px', fontWeight: '700',
              background: activeTaskTab === 'for_checking' ? '#fef08a' : 'var(--background-secondary)',
              color: activeTaskTab === 'for_checking' ? '#854d0e' : 'var(--text-tertiary)',
              padding: '1px 8px', borderRadius: '10px'
            }}>
              {assignments.filter(a => a.status !== 'completed' && a.recent_submissions?.length > 0 && (teamFilter === 'all' || (a.team || 'IT Dept') === teamFilter)).length}
            </span>
        ...
        {(() => {
          const tabFiltered = assignments.filter(a => {
            ...
            if (activeTaskTab === 'done') {
              tabMatch = a.status === 'completed';
            } else if (activeTaskTab === 'for_checking') {
              tabMatch = a.status !== 'completed' && a.recent_submissions?.length > 0;
            } else {
              tabMatch = a.status !== 'completed' && !(a.recent_submissions?.length > 0);
            }
          ...
          <div className="tl-empty-state">
            <div className="tl-empty-state-icon">{activeTaskTab === 'done' ? '✅' : activeTaskTab === 'for_checking' ? '🔍' : '📋'}</div>
            <h3>{searchQuery ? 'No Results Found' : activeTaskTab === 'done' ? 'No completed tasks yet' : activeTaskTab === 'for_checking' ? 'No tasks for checking' : 'No tasks yet'}</h3>
            <p>{searchQuery ? `No tasks match "${searchQuery}".` : activeTaskTab === 'done' ? 'Tasks marked as done will appear here.' : activeTaskTab === 'for_checking' ? 'Tasks pending your review will appear here.' : 'Create your first task to get started'}</p>
