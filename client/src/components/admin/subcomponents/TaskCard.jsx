...
const TaskCard = ({
  ...
  // Check if task is completed (all files approved)
  ...
  // Use assigned_member_details as provided by coreController.js
  const memberNames = useMemo(() => {
    if (assignment.assigned_to === 'all') return 'all members';
    
    // Check both members (from my previous service change) and assigned_member_details (from coreController)
    const memberList = assignment.assigned_member_details || assignment.members || [];
    
    if (memberList.length === 0) {
      return assignment.team_leader_fullname || assignment.team_leader_username || 'Unknown';
    }
  ...
  return (
    ...
    >
        ...
        <div className="header-fidelity-right">
          {isTaskCompleted ? (
            ...
            <div className="due-info-fidelity">
              ...
              <span className="days-left-fidelity" style={{ color: '#22c55e' }}>
                ...
                {individualFiles.map(file => (
                  <div key={file.id} className="file-card-fidelity" onClick={() => handleOpenFile(file.file_path, file.id)}>
                    <div className="file-card-icon">
                      <FileIcon filename={file.original_name} size="small" />
                    </div>
      ...
      <div className="card-fidelity-footer">
        <div className="comments-fidelity-row">
        ...
        <div className="menu-fidelity-container">
          ...
          {showMenuForAssignment === assignment.id && (
            <div className="fidelity-dropdown">
               <button onClick={() => { setAssignmentToDelete(assignment); setShowDeleteModal(true); setShowMenuForAssignment(null); }}>Delete</button>
            </div>
          )}
        </div>
      </div>
