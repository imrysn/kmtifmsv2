...
const UserPerformanceCard = memo(({ user, performanceData, fallbackStats, isCollapsible = false, onPerformanceLoad, isLoading = false }) => {
  ...
  const checkingAssigned = performance?.checkingAssigned ?? (fallbackStats?.checkingAssigned || 0);
  const checkingCompleted = performance?.checkingCompleted ?? (fallbackStats?.checkingCompleted || 0);
  const checkingCompletionRate = performance?.checkingCompletionRate ?? (fallbackStats?.checkingCompletionRate || 0);
