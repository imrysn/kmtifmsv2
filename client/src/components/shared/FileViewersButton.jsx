...
const FileViewersButton = ({ fileId, size = 14 }) => {
  ...
  useEffect(() => {
    if (!fileId) return
    fetch(`${API_BASE_URL}/api/files/${fileId}/views`)
      .then(r => r.json())
  ...
  const fetchViewers = async () => {
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE_URL}/api/files/${fileId}/views`)
  ...
  const getRoleColor = (role) => {
    ...
    if (r.includes('TEAM_LEADER') || r.includes('TEAM LEADER')) return '#1d4ed8'
    return '#059669'
  }
  ...
  const getRoleLabel = (role) => {
    ...
    if (r.includes('TEAM_LEADER') || r.includes('TEAM LEADER')) return 'Team Leader'
    return 'User'
  }
