...
async function updateBatchStatus(fileIds, updates) {
  ...
  if (current_stage !== undefined) {
    fields.push('current_stage = ?'); values.push(current_stage);
  }
  if (admin_id !== undefined) {
    fields.push('admin_id = ?, admin_reviewed_at = NOW()'); values.push(admin_id);
  }
  if (admin_username !== undefined) {
    fields.push('admin_username = ?'); values.push(admin_username);
  }
  if (admin_comments !== undefined) {
    fields.push('admin_comments = ?'); values.push(admin_comments);
  }
...
async function updatePriority(fileId, priority, dueDate) {
  ...
  if (dueDate !== undefined) {
    fields.push('due_date = ?'); params.push(dueDate);
  }
  if (fields.length === 0) {
    return false;
  }
