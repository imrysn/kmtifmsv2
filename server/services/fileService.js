...
async function uploadFile(fileData, user) {
  ...
  if (assignmentId && !fileData.folder_name) {
    ...
    if (!pathSource) {
      ...
      if (historical && historical.folder_name) {
        pathSource = historical;
      }
    }
    if (pathSource) {
    ...
    try {
      ...
      if (assignment && assignment.team_leader_id) {
        ...
        try {
          const checkerIds = (() => {
            try {
              return JSON.parse(assignment.checker_ids || '[]').map(String);
            } catch {
              return [];
            }
          })();
    ...
    try {
      ...
      if (assignmentId && !fileData.folder_name) {
        ...
        if (existing && existing.folder_name) {
          ...
          if (pathSource && !pathSource.folder_name) {
            pathSource = null;
          }
        }
      ...
      if (skipCreate) {
        ...
        if (fileData.isThisFileRevision) {
          dbData.status = 'under_revision';
        } else if (!dbData.status || dbData.status === '') {
          dbData.status = 'uploaded';
        }
...
async function openFile(fileId) {
  ...
  if (!resolved || !resolved.path) {
    throw new ValidationError('File path could not be resolved');
  }
...
async function resolvePhysicalPath(fileId, requestedType = null, folderName = null) {
  ...
  if (file.public_network_url && !file.public_network_url.startsWith('http')) {
    if (await diskExists(file.public_network_url)) {
      return returnAndCache(file.public_network_url);
    }
  }
      ...
      try {
        ...
        if (await diskExists(grandparentDir)) {
          ...
          if (matchedDir) {
            ...
            if (deepWinner) {
              return returnAndCache(deepWinner);
            }
          }
        }
      } catch (e) {
  ...
  if (file.file_path && file.file_path.startsWith('/uploads/') && !isTempPath(file.file_path)) {
    ...
    if (await diskExists(candidate)) {
      return returnAndCache(candidate);
    }
  }
      ...
      try {
        const deepScan = async (dir, depth = 0) => {
          ...
          for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
              if (folderName && entry.name === folderName) {
                return fullPath;
              }
  ...
  if (!isAttachment && file.username) {
    ...
    if (file.is_folder || file.folder_name) {
      try {
        ...
        if (scanWinner) {
          return returnAndCache(scanWinner);
        }
      } catch (e) {
  ...
  if (fallback.startsWith('/uploads/')) {
    fallback = path.join(uploadsDir, fallback.replace(/^\/uploads\//, ''));
  } else if (fallback && !path.isAbsolute(fallback)) {
    fallback = path.join(uploadsDir, fallback);
  }
  ...
  for (let i = 0; i < fileIds.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (fileId) => {
      try {
        if (action === 'approve') {
          if (user.role === 'ADMIN') {
            await approveByAdmin(fileId, user, comments);
          } else {
            await approveByTeamLeader(fileId, user, comments);
          }
        } else if (action === 'reject') {
          if (user.role === 'ADMIN') {
            await rejectByAdmin(fileId, user, comments);
          } else {
            await rejectByTeamLeader(fileId, user, comments);
          }
        }
...
async function filterTeamLeaderQueue(team, filters = {}, sort = {}, page = 1, limit = 50) {
  ...
  if (filters.fileType?.length) {
    whereClauses.push(`file_type IN (${filters.fileType.map(() => '?').join(',')})`); params.push(...filters.fileType);
  }
  if (filters.submittedBy?.length) {
    whereClauses.push(`user_id IN (${filters.submittedBy.map(() => '?').join(',')})`); params.push(...filters.submittedBy);
  }
  if (filters.dateFrom) {
    whereClauses.push('uploaded_at >= ?'); params.push(filters.dateFrom);
  }
  if (filters.dateTo)   {
    whereClauses.push('uploaded_at <= ?'); params.push(filters.dateTo);
  }
  if (filters.priority) {
    whereClauses.push('priority = ?'); params.push(filters.priority);
  }
  if (filters.hasDeadline) {
    whereClauses.push('due_date IS NOT NULL');
  }
  if (filters.isOverdue) {
    whereClauses.push('due_date < ?'); params.push(new Date().toISOString());
  }
...
async function moveFolderToNas({ folderName, username, fileIds, destinationPath, comments }, admin) {
  const dbFiles = (await Promise.all(
    fileIds.map(async (fileId) => {
      const row = await queryOne('SELECT * FROM files WHERE id = ?', [fileId]);
      if (row) {
        return row;
      }
        ...
        if (!sourceExists && dbFiles[0]?.file_path) {
          ...
          if (parts.length >= 2) {
            ...
            if (await fs.access(alt).then(() => true).catch(() => false)) {
              sourceFolderPath = alt; sourceExists = true;
            }
          }
        }
        ...
        if (!sourceExists) {
          ...
          if (topFolder) {
            ...
            if (await fs.access(alt2).then(() => true).catch(() => false)) {
              sourceFolderPath = alt2; sourceExists = true;
            }
          }
        }
          ...
          const copyDir = async (src, dest) => {
            ...
            for (let i = 0; i < entries.length; i += BATCH) {
              await Promise.all(entries.slice(i, i + BATCH).map(async entry => {
                ...
                if (entry.isDirectory()) {
                  await fs.mkdir(destPath, { recursive: true }); await copyDir(srcPath, destPath);
                } else {
