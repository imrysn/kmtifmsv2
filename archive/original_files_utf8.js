...
// 笏笏 Manual file sync endpoint 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
router.post('/sync-deleted', async (req, res) => {
  try {
    console.log('売 [Sync] Manual sync triggered');
    const summary = await syncDeletedFiles(uploadsDir, networkDataPath);
    res.json({
      success: true,
      message: `Sync complete 窶・removed ${summary.removed} orphaned record(s)`,
      ...summary
    });
  } catch (err) {
    console.error('笶・[Sync] Error:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// One-time fix: promote any files uploaded by a TEAM_LEADER that are still stuck on
// 'uploaded' / 'pending_team_leader' 窶・they should skip TL review and go to Pending Admin.
;(async function fixTeamLeaderUploadedFiles() {
  try {
    ...
    console.log(`笨・TL uploaded files fix applied 窶・${affected} row(s) updated`)
  } catch (err) {
    console.warn('笞・・TL uploaded files fix failed:', err.message)
  }
})()
  ...
  for (const col of cols) {
    try {
      ...
      if (!exists || exists.length === 0) {
        await query(col.sql);
        console.log(`笨・Added column assignment_attachments.${col.name}`);
      }
    } catch (err) {
      console.warn(`笞・・Could not add column ${col.name}:`, err.message);
    }
  }
})()
...
router.post('/folder/move-to-nas', async (req, res) => {
  ...
  try {
    // Load all files from DB 窶・check both files table AND assignment_attachments
    const dbFiles = await Promise.all(
      fileIds.map(fileId => new Promise((resolve, reject) => {
        db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, row) => {
          if (err) return reject(err)
          if (row) return resolve(row)
          // Not in files table 窶・check assignment_attachments
    ...
    if (!isAttachmentFolder) {
      ...
      if (sourceExists) {
        ...
        console.log(`笨・Folder copied to NAS and source deleted: ${destFolderPath}`)
      } else {
        console.warn(`笞・・Source folder not found at ${sourceFolderPath}, will copy file-by-file from DB paths`)
      }
    } else {
      console.log(`搭 Attachment folder 窶・copying files individually from DB paths`)
    }
    ...
    for (const file of dbFiles) {
      ...
      if (!sourceExists) {
        ...
        if (path.isAbsolute(file.file_path) || file.file_path.startsWith('\\\\')) {
          // Already an absolute path 窶・use as-is
        ...
        console.log(`剥 Attachment src: ${srcFilePath}`)
        console.log(`剥 Attachment dst: ${nasFilePath}`)
        ...
        if (srcOk) {
          ...
          if (normalSrc === normalDst) {
            console.log(`邃ｹ・・Source and destination are the same path, skipping copy: ${srcFilePath}`)
          } else {
            await fs.copyFile(srcFilePath, nasFilePath)
            await safeDeleteFile(srcFilePath)
            console.log(`笨・Copied file to NAS: ${nasFilePath}`)
          }
        } else {
          console.warn(`笞・・Source file not found at: ${srcFilePath} 窶・may already have been moved on a previous approval`)
        }
      }
    ...
    // If ALL files for a user are approved 竊・single folder-level notification.
    // If MOST files are rejected but a few approved 竊・individual notifications for each approved.
    // If MOST files are approved but a few rejected 竊・individual notifications for each rejected.
    // Individual file notifications are always sent when the minority count is 竕､ 2.
    const userFileMap = {};
    for (const file of dbFiles) {
    ...
    for (const [userId, { approved, rejected }] of Object.entries(userFileMap)) {
      ...
      if (rejectedCount === 0) {
        // All approved 窶・send one folder-level notification
        createNotification(
        ...
        // All rejected 窶・send one folder-level rejection notification
        createNotification(
        ...
        // Majority rejected, minority approved 窶・individual notifications for approved only
        ...
        // Majority approved, minority rejected 窶・individual notifications for rejected only
        ...
        // Mixed, neither side is a small minority 窶・send grouped notifications for each outcome
        createNotification(
          userId, null, 'final_approval',
    ...
    console.log(`笨・Folder "${folderName}" fully approved and on NAS`)
    res.json({
      success: true,
    ...
    console.error('笶・Error moving folder to NAS:', error)
    res.status(500).json({
      success: false,
...
router.post('/check-duplicate', (req, res) => {
  ...
  console.log(`剥 Checking for duplicate file: ${originalName} by user ${userId}`);

  db.get('SELECT * FROM files WHERE original_name = ? AND user_id = ?', [originalName, userId], (err, existingFile) => {
    if (err) {
      console.error('笶・Error checking for duplicate file:', err);
...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    if (autoReplace) {
      console.log(`統 Auto-replacing previously rejected file (ID: ${previouslyRejected.id})`);
    }
    ...
    console.log(`刀 Upload by ${username} (${userTeam}): ${originalFilename}${isRevision === 'true' ? ' [REVISION]' : ''}`);
    ...
    try {
      ...
      console.error('笶・Error moving file to user folder:', moveError.message);
    ...
    // IMPORTANT: Match on folder_name too 窶・a file inside a folder and a standalone file with the same
    ...
    if (replaceExisting !== 'true' && !autoReplace) {
      ...
      db.get(dupQuery, dupParams, async (err, existingFile) => {
        if (err) {
          console.error('笶・Error checking for duplicate:', err);
          await safeDeleteFile(req.file.path);
          return res.status(500).json({
            success: false,
            message: 'Failed to check for duplicate files'
      ...
      // Match on folder_name so folder files don窶冲 replace standalone files with the same name
        ...
        if (existingFile) {
          ...
          WHERE id = ?`,
            [
              ...
              if (updateErr) {
                console.error('笶・Error updating file record:', updateErr);
                await safeDeleteFile(req.file.path);
                return res.status(500).json({
                  success: false,
              ...
              console.log(`笨・File ${action} (ID: ${fileId})`);
              ...
              if (userRole && userRole.toUpperCase().includes('TEAM_LEADER')) {
                createAdminNotification(
                  ...
                  `${username} (Team Leader) ${isRevision === 'true' ? 'revised' : 'uploaded'} "${req.file.originalname}" 窶・pending review.`,
    ...
    function insertFileRecord() {
      ...
      // Team Leaders skip the TL review stage 窶・their files go straight to Pending Admin
      ...
      db.run(`INSERT INTO files (
        ...
        [
          ...
          if (err) {
            console.error('笶・Error saving file to database:', err);
    ...
    console.error('笶・Error handling file upload:', error);
    res.status(500).json({
      success: false,
  ...
  db.get('SELECT file_path, original_name, public_network_url, status FROM files WHERE id = ?', [id], (err, file) => {
    ...
    if (!file) {
      ...
      return db.get('SELECT file_path, original_name, public_network_url, status FROM assignment_attachments WHERE id = ?', [id], (err2, attachment) => {
        if (err2) {
          console.error('笶・Error checking attachment:', err2);
          return res.status(500).json({
            success: false,
        ...
        if (attachment.public_network_url && !attachment.public_network_url.startsWith('http')) {
          console.log(`笨・Using public_network_url for attachment: ${attachment.public_network_url}`);
          return res.json({
            success: true,
        ...
        // Do NOT call path.resolve() on UNC paths 窶・it corrupts them
        if (absolutePath && (absolutePath.startsWith('\\\\') || /^[A-Za-z]:[\\/]/.test(absolutePath))) {
          console.log(`笨・File path is already absolute (attachment): ${absolutePath}`);
        ...
        console.log(`笨・File path resolved (attachment) for ID ${id}: ${absolutePath}`);
    ...
    if (file.public_network_url && !file.public_network_url.startsWith('http')) {
      ...
      console.log(`藤 Using public network URL for file: ${absolutePath}`);
    } else {
      // If it's already a UNC path (\\server\...) or absolute Windows path, use as-is
      // Do NOT call path.resolve() on UNC paths 窶・it corrupts them
      if (absolutePath && (absolutePath.startsWith('\\\\') || /^[A-Za-z]:[\\/]/.test(absolutePath))) {
        console.log(`笨・File path is already absolute: ${absolutePath}`);
    ...
    console.log(`笨・File path resolved for ID ${id}: ${absolutePath}`);
...
router.get('/member/:memberId', (req, res) => {
  ...
  console.log(`塘 Getting files for team member ${memberId}`);
  ...
  db.all(
    ...
    (err, files) => {
      if (err) {
        console.error('笶・Error getting member files:', err);
        return res.status(500).json({
          success: false,
          message: 'Failed to fetch member files'
        });
      }
      console.log(`笨・Retrieved ${files.length} files for member ${memberId}`);
      res.json({
        success: true,
        files
      });
...
router.get('/user/:userId', (req, res) => {
  ...
  console.log(`刀 Getting files for user ${userId} - Page ${page}, Limit ${limit}`);

  // Get total count
  db.get('SELECT COUNT(*) as total FROM files WHERE user_id = ?', [userId], (err, countResult) => {
    if (err) {
      console.error('笶・Error getting user file count:', err);
      return res.status(500).json({
        success: false,
    ...
    db.all(
      ...
      (err, files) => {
        if (err) {
          console.error('笶・Error getting user files:', err);
          return res.status(500).json({
            success: false,
            message: 'Failed to fetch files'
          });
        }
        console.log(`笨・Retrieved ${files.length} files for user ${userId}`);
        res.json({
          success: true,
          files,
          pagination: {
...
router.get('/user/:userId/pending', (req, res) => {
  ...
  console.log(`刀 Getting pending files for user ${userId} - Page ${page}, Limit ${limit}`);

  // Get total count
  db.get('SELECT COUNT(*) as total FROM files WHERE user_id = ?', [userId], (err, countResult) => {
    if (err) {
      console.error('笶・Error getting user pending file count:', err);
      return res.status(500).json({
        success: false,
    ...
    db.all(
      ...
      (err, files) => {
        if (err) {
          console.error('笶・Error getting user pending files:', err);
          return res.status(500).json({
            success: false,
            message: 'Failed to fetch pending files'
          });
        ...
        console.log(`笨・Retrieved ${processedFiles.length} pending files for user ${userId}`);
        res.json({
          success: true,
...
router.get('/team-leader/:team', (req, res) => {
  ...
  console.log(`刀 Getting files for team leader review: ${team} team - Page ${page}, Limit ${limit}`);

  // Get total count
  db.get('SELECT COUNT(*) as total FROM files WHERE user_team = ? AND current_stage = ?', [team, 'pending_team_leader'], (err, countResult) => {
    if (err) {
      console.error('笶・Error getting team leader file count:', err);
      return res.status(500).json({
        success: false,
    ...
    db.all(
      ...
      (err, files) => {
        if (err) {
          console.error('笶・Error getting team leader files:', err);
          return res.status(500).json({
            success: false,
            message: 'Failed to fetch files for review'
          });
        }
        console.log(`笨・Retrieved ${files.length} files for ${team} team leader review`);
        res.json({
          success: true,
          files,
          pagination: {
...
router.get('/admin', (req, res) => {
  ...
  console.log(`刀 Getting files for admin review - Page ${page}, Limit ${limit}`);

  // Get total count
  db.get('SELECT COUNT(*) as total FROM files WHERE current_stage = ?', ['pending_admin'], (err, countResult) => {
    if (err) {
      console.error('笶・Error getting admin file count:', err);
      return res.status(500).json({
        success: false,
    ...
    db.all(
      ...
      (err, files) => {
        if (err) {
          console.error('笶・Error getting admin files:', err);
          return res.status(500).json({
            success: false,
            message: 'Failed to fetch files for admin review'
          });
        }
        console.log(`笨・Retrieved ${files.length} files for admin review`);
        res.json({
          success: true,
          files,
          pagination: {
...
router.get('/all', (req, res) => {
  console.log(`刀 Getting all files (admin view)`);
  ...
  db.all(
    ...
    (err, files) => {
      if (err) {
        console.error('笶・Error getting all files:', err);
        return res.status(500).json({
          success: false,
      ...
      db.all(
        ...
        (err2, attachments) => {
          if (err2) {
            console.warn('笞・・Could not fetch assignment attachments:', err2.message);
            // Still return regular files even if attachments fail
            console.log(`笨・Retrieved ${files.length} files (all files view, attachments unavailable)`);
            return res.json({ success: true, files });
          }

          // Merge 窶・avoid duplicates (attachment already in files table)
          ...
          console.log(`笨・Retrieved ${files.length} files + ${newAttachments.length} TL attachments (all files view)`);
...
router.post('/:fileId/team-leader-review', (req, res) => {
  ...
  console.log(`搭 Team leader ${action} for file ${fileId} by ${teamLeaderUsername}`);
  ...
  db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, file) => {
    if (err) {
      console.error('笶・Error getting file:', err);
      return res.status(500).json({
        success: false,
    ...
    db.run(tlSql, tlParams, function (err) {
      if (err) {
        console.error('笶・Error updating file status:', err);
      ...
      if (comments) {
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              console.error('笶・Error adding team leader comment:', err);
              console.error('Comment details:', { fileId, teamLeaderId, teamLeaderUsername, teamLeaderRole, comments, commentType });
            } else {
              console.log('笨・Team leader comment added successfully');
              ...
              createNotification(
                ...
                console.error('笶・Failed to create comment notification:', err);
              });
            }
          }
        );
      ...
      console.log(`笨・File ${action}d by team leader: ${file.filename}`);
      res.json({
        success: true,
...
router.post('/:fileId/move-to-projects', async (req, res) => {
  ...
  console.log(`逃 Moving file ${fileId} to destination: ${destinationPath}`);

  try {
    // Get file info 窶・check files table first, then assignment_attachments
    ...
    // Get source file path 窶・handle both absolute (attachment) and relative paths.
    ...
    if (file.file_path && file.file_path.startsWith('/uploads/')) {
      // Relative path stored in DB 窶・resolve against the NAS uploads directory
    ...
    if (!sourceExists) {
      console.error('笶・Source file not found:', sourcePath);
      return res.status(404).json({
        success: false,
    ...
    // The admin already chose the exact destination folder 窶・no extra wrapping.
    const effectiveDestPath = destinationPath;

    console.log('唐 Destination path:', effectiveDestPath);

    // Ensure destination directory exists
    await fs.mkdir(effectiveDestPath, { recursive: true });

    // Place file using just its original_name 窶・no subfolder structure
    ...
    console.log('笨・File copied to:', destinationFilePath);

    // Delete file from uploads folder if requested
    if (deleteFromUploads) {
      const deleteResult = await safeDeleteFile(sourcePath);
      if (deleteResult.success) {
        console.log('笨・File deleted from uploads folder:', sourcePath);
      } else {
        console.warn('笞・・Could not delete file from uploads folder:', deleteResult.message);
      }
    }

    // Update database with new path 窶・write to correct table
    if (isAttachment) {
      await new Promise((resolve, reject) => {
        db.run(
    ...
    console.log(`笨・File moved successfully: ${file.original_name}`);
    res.json({
      success: true,
    ...
    console.error('笶・Error moving file to projects:', error);
    res.status(500).json({
      success: false,
...
router.post('/:fileId/admin-review', async (req, res) => {
  ...
  console.log(`搭 Admin ${action} for file ${fileId} by ${adminUsername}`);
  ...
  // Get current file status 窶・check files table first, then assignment_attachments
  db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, file) => {
    ...
    if (!file) {
      ...
      return db.get('SELECT *, created_at AS uploaded_at, uploaded_by_id AS user_id, uploaded_by_username AS username FROM assignment_attachments WHERE id = ?', [fileId], (err2, attachment) => {
        ...
        db.run(updateSql, updateParams, function(updateErr) {
          if (updateErr) {
            console.warn('笞・・Could not update attachment status (columns may not exist yet):', updateErr.message);
            // Still return success 窶・the columns may not be migrated yet
          }
    ...
    db.run(adminSql, adminParams, function (err) {
      if (err) {
        console.error('笶・Error updating file status (admin):', err);
        return res.status(500).json({
          success: false,
          message: 'Failed to update file status',
          error: err.message
      ...
      console.log(`笨・Database updated successfully. Rows affected: ${this.changes}`);
      ...
      if (comments) {
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              console.error('笶・Error adding admin comment:', err);
              console.error('Comment details:', { fileId, adminId, adminUsername, adminRole, comments, commentType });
            } else {
              console.log('笨・Admin comment added successfully');
              ...
              createNotification(
                ...
                console.error('笶・Failed to create admin comment notification:', err);
              });
            }
          }
        );
      ...
      function sendTeamLeaderNotification(teamLeaderId) {
        if (teamLeaderId && teamLeaderId !== adminId) {
          ...
          console.log(`透 Created admin ${action} notification for team leader ID: ${teamLeaderId}`);
        }
      }
      ...
      console.log(`笨・File ${action}d by admin: ${file.filename}${action === 'approve' ? ' - Published to Public Network' : ''}`);
      res.json({
        success: true,
...
router.get('/:fileId/comments', (req, res) => {
  ...
  db.all(
    ...
    (err, comments) => {
      if (err) {
        console.error('笶・Error getting file comments:', err);
        return res.status(500).json({
          success: false,
...
router.post('/:fileId/comments', (req, res) => {
  ...
  db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, file) => {
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          console.error('笶・Error adding comment:', err);
          return res.status(500).json({
            success: false,
...
router.get('/:fileId/history', (req, res) => {
  ...
  db.all(
    ...
    (err, history) => {
      if (err) {
        console.error('笶・Error getting file history:', err);
        return res.status(500).json({
          success: false,
...
router.delete('/:fileId', async (req, res) => {
  ...
  console.log(`卵・・Deleting file ${fileId} by admin ${adminUsername}`);
  ...
  try {
    ...
    if (!file) {
      // Not in files table 窶・check assignment_attachments (old TL-attached files)
      ...
      console.log(`笨・Attachment deleted: ${attachment.original_name}`);
    ...
    try {
      ...
      for (const sub of submissions) {
        await new Promise((resolve) => {
          db.run(
            ...
            (err) => {
              if (err) console.warn('笞・・Could not reset assignment_members status:', err.message);
              resolve();
            }
          );
        });
      ...
      if (deletedCount > 0) {
        console.log(`笨・Cleared ${deletedCount} assignment submission(s) for file ${fileId} 窶・user can resubmit`);
      }
    } catch (clearError) {
      console.error('笞・・CRITICAL: Failed to clear assignment submissions:', clearError.message);
      return res.status(500).json({
        success: false,
    ...
    // IMPORTANT: Check public_network_url FIRST 窶・approved files are moved to NAS and their
    ...
    if (file.public_network_url && !file.public_network_url.startsWith('http')) {
      // File was approved and moved to NAS 窶・use the NAS path directly
      filePath = file.public_network_url;
      console.log(`刀 File is on NAS, using public_network_url: ${filePath}`);
    } else if (file.file_path) {
      // File is still in uploads staging area 窶・resolve relative path
    ...
    console.log(`卵・・Attempting to delete file at: ${filePath}`);
    const deleteResult = await safeDeleteFile(filePath);

    if (!deleteResult.success && !deleteResult.notFound) {
      console.warn(`笞・・Physical file deletion issue: ${deleteResult.message}`);
    }
  ...
  try {
    ...
    db.get('SELECT * FROM files WHERE id = ?', [id], async (err, file) => {
      ...
      if (file.public_network_url) {
        ...
        console.log(`卵・・File has been moved to projects, deleting from: ${resolved}`);
      }
      ...
      console.log(`卵・・Attempting to delete physical file: ${resolved}`);

      const deleteResult = await safeDeleteFile(resolved);

      if (deleteResult.success) {
        if (deleteResult.notFound) {
          console.log(`邃ｹ・・Physical file not found (already removed): ${resolved}`);
          return res.json({ success: true, message: 'Physical file not found (already removed)' });
        }

        console.log(`笨・Physical file deleted successfully: ${resolved}`);
        return res.json({ success: true, message: 'Physical file deleted' });
      } else {
        console.error('笶・Error deleting physical file:', resolved, deleteResult.error);
        return res.status(500).json({
          success: false,
    ...
    console.error('笶・delete-file error:', err);
    return res.status(500).json({ success: false, message: 'Server error', detail: err.message });
  }
});

// REMOVED: Duplicate DELETE /:id route 窶・handled by DELETE /:fileId above (full audit trail).
...
router.post('/bulk-action', (req, res) => {
  ...
  console.log(`搭 Bulk ${action} for ${fileIds.length} files by ${reviewerUsername}`);
  console.log(`剥 DEBUG bulk-action: reviewerRole='${reviewerRole}', reviewerId=${reviewerId}, fileIds=${JSON.stringify(fileIds)}`);
  ...
  fileIds.forEach(fileId => {
    db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, file) => {
      ...
      console.log(`剥 DEBUG file ${fileId}: normalizedRole='${normalizedRole}', isTeamLeader=${isTeamLeader}, file.current_stage='${file.current_stage}', correctStage=${correctStage}`);
      ...
      db.run(updateSql, updateParams, function (err) {
        if (err) {
          console.error(`笶・Error updating file ${fileId}:`, err);
  ...
  function checkComplete() {
    if (processed === fileIds.length) {
      ...
      console.log(`笨・Bulk action complete: ${results.success.length} succeeded, ${results.failed.length} failed`);
      res.json({
        success: true,
...
router.post('/team-leader/:team/filter', (req, res) => {
  ...
  console.log(`剥 Filtering files for team ${team}:`, filters);
  ...
  db.get(countSql, params, (err, countResult) => {
    if (err) {
      console.error('笶・Error counting filtered files:', err);
      return res.status(500).json({ success: false, message: 'Failed to filter files' });
    }

    db.all(dataSql, [...params, limit, offset], (err, files) => {
      if (err) {
        console.error('笶・Error getting filtered files:', err);
        return res.status(500).json({ success: false, message: 'Failed to filter files' });
      }

      console.log(`笨・Retrieved ${files.length} filtered files`);
      res.json({
        success: true,
        files,
        pagination: {
...
router.patch('/:fileId/priority', (req, res) => {
  ...
  console.log(`識 Setting priority for file ${fileId}: ${priority}, due: ${dueDate}`);
  ...
  db.run(sql, params, function (err) {
    if (err) {
      console.error('笶・Error updating file priority:', err);
    ...
    console.log('笨・File priority updated');
...
router.get('/notifications/:team', (req, res) => {
  const { team } = req.params;
  console.log(`粕 Getting notifications for team ${team}`);
  ...
  db.all(
    ...
    (err, files) => {
      if (err) {
        console.error('笶・Error getting notifications:', err);
      ...
      console.log(`笨・Retrieved ${notifications.length} notifications`);
      res.json({
        success: true,
        notifications,
        counts: {
...
router.get('/team/:team/status/:status', (req, res) => {
  ...
  console.log(`投 Getting ${status} files for team ${team}`);
  ...
  db.all(
    ...
    (err, files) => {
      if (err) {
        console.error(`笶・Error getting ${status} files:`, err);
        return res.status(500).json({
          success: false,
          message: `Failed to fetch ${status} files`
        });
      }
      console.log(`笨・Retrieved ${files.length} ${status} files for team ${team}`);
      res.json({
        success: true,
        files
      });
...
router.post('/open-file', async (req, res) => {
  ...
  console.log(`唐 Opening file with default application: ${filePath}`);
  ...
  try {
    ...
    try {
      ...
      console.error('笶・File not found:', resolvedPath);
      return res.status(404).json({
        success: false,
        message: 'File not found'
      });
    ...
    // Sanitize path to prevent shell injection 窶・reject any path containing shell metacharacters
    if (/[&;`|<>$!\r\n]/.test(resolvedPath)) {
    ...
    exec(command, (error) => {
      if (error) {
        console.error('笶・Error opening file:', error);
        return res.status(500).json({
          success: false,
      ...
      console.log('笨・File opened successfully with default application');
      res.json({ success: true, message: 'File opened successfully' });
    });
  } catch (error) {
    console.error('笶・Error processing file open request:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to open file',
      error: error.message
...
router.post('/folder/delete-attachments', async (req, res) => {
  ...
  console.log(`卵・・Deleting attachment folder "${folderName}" (${fileIds?.length || 0} files) by ${adminUsername}`)
  ...
  try {
    ...
    console.error('笶・Error deleting attachment folder:', error)
...
router.post('/folder/delete', async (req, res) => {
  ...
  console.log(`卵・・Deleting folder "${folderName}" by ${username}`);

  try {
    // Step 1: Fetch all file records from DB so we know the REAL physical paths
    // (files may have been moved to NAS after approval 窶・file_path/public_network_url reflects that)
    ...
    if (fileIds && fileIds.length > 0) {
      ...
      for (const physicalPath of deletedPaths) {
        ...
        if (result.success) {
          console.log(`笨・Deleted physical file: ${physicalPath}`);
        } else if (!result.notFound) {
          console.warn(`笞・・Could not delete: ${physicalPath} 窶・${result.message}`);
        }
      }
    ...
    for (const dirPath of dirsToDelete) {
      try {
        ...
        if (exists) {
          ...
          console.log(`笨・Deleted folder directory: ${dirPath}`);
        } else {
          console.log(`邃ｹ・・Folder directory not found (already gone): ${dirPath}`);
        }
      } catch (dirErr) {
        console.warn(`笞・・Could not delete directory ${dirPath}: ${dirErr.message}`);
      }
    }
    ...
    console.error('笶・Error deleting folder:', error);
    res.status(500).json({
      success: false,
...
// Download a file by ID 窶・same path resolution as /stream but forces browser download
router.get('/:fileId/download', async (req, res) => {
  ...
  try {
    ...
    console.log(`筮・ｸ・Downloading file to client: ${filePath}`);
    const stream = fsSync.createReadStream(filePath);
    stream.on('error', (err) => {
      console.error('笶・Download stream error:', err);
      if (!res.headersSent) res.status(500).send('Error reading file');
    });
    stream.pipe(res);
  } catch (error) {
    console.error('笶・Error downloading file:', error);
...
router.get('/folder/zip', async (req, res) => {
  ...
  try {
    ...
    console.log(`逃 Streaming folder zip: ${zipPath}`);
    ...
    zipStream.on('error', (err) => {
      console.error('笶・Zip stream error:', err);
      if (!res.headersSent) res.status(500).send('Error streaming zip');
    });
    zipStream.pipe(res);
  } catch (error) {
    console.error('笶・Error creating folder zip:', error);
...
// Force-download a file by ID 窶・identical path resolution to /stream but sends Content-Disposition: attachment
router.get('/:fileId/download', async (req, res) => {
  const { fileId } = req.params;
  if (!/^\d+$/.test(fileId)) return res.status(400).send('Invalid file ID');
  ...
  try {
    ...
    console.log(`筮・ｸ・Force-downloading file: ${filePath}`);
    const stream = fsSync.createReadStream(filePath);
    stream.on('error', (err) => {
      console.error('笶・Download stream error:', err);
      if (!res.headersSent) res.status(500).send('Error reading file');
    ...
    console.error('笶・Error in download route:', error);
    if (!res.headersSent) res.status(500).send('Server error');
  }
});

// Stream a file by ID 窶・reads from NAS/local path and serves inline to the browser
...
router.get('/:fileId/stream', async (req, res) => {
  ...
  try {
    ...
    const fsSync = require('fs');
    if (!fsSync.existsSync(filePath)) {
      // For UNC paths existsSync can lie 窶・try reading anyway
      try {
        const stat = fsSync.statSync(filePath);
    ...
    console.log(`藤 Streaming file to client: ${filePath}`);
    const stream = fsSync.createReadStream(filePath);
    stream.on('error', (err) => {
      console.error('笶・Stream error:', err);
      if (!res.headersSent) res.status(500).send('Error reading file');
    });
    stream.pipe(res);
  } catch (error) {
    console.error('笶・Error streaming file:', error);
    if (!res.headersSent) res.status(500).send('Server error');
  }
});
...
router.get('/:fileId', (req, res) => {
  ...
  console.log(`統 Getting file details for ID: ${fileId}`);
  ...
  db.get('SELECT * FROM files WHERE id = ?', [fileId], (err, file) => {
    ...
    if (file) {
      console.log('笨・File found in files table:', file.original_name);
      return res.json({ success: true, file });
    }

    // Not in files table 窶・check assignment_attachments (TL-attached files)
    console.log(`梼 File ${fileId} not in files table, checking assignment_attachments...`);
    db.get(
      ...
      (err2, attachment) => {
        if (err2) {
          console.error('笶・Error checking assignment_attachments:', err2);
          return res.status(500).json({ success: false, message: 'Failed to get file details' });
        }

        if (!attachment) {
          console.log('笶・File not found in either table:', fileId);
          return res.status(404).json({ success: false, message: 'File not found' });
        }

        console.log('笨・File found in assignment_attachments:', attachment.original_name);
